...
class MSDeformAttnPixelDecoder(nn.Module):
    ...
    def forward_features(self, features):
        ...
        # import mmcv
        # temp = srcs[0]*255
        # temp = torch.mean(temp[0],dim=0).unsqueeze(0).permute(1,2,0).cpu().detach().numpy()
        
        # import matplotlib.pyplot as plt
        # plt.imsave('3.jpg', temp[:,:,0], cmap='jet')
        ...
        # import mmcv
        # temp = torch.mean(out[-1][0],dim=0).unsqueeze(0).permute(1,2,0).cpu().detach().numpy()*255
        # mmcv.imwrite(temp.astype(np.uint8),'2.jpg')
        # exit(0)
