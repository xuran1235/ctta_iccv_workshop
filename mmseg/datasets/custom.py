...
class CustomDataset(Dataset):
    ...
    def get_gt_seg_maps(self, efficient_test=False):
        ...
        for img_info in self.img_infos:
            ...
            if gt_seg_map.shape[2]==3:
                gt_seg_map = gt_seg_map[...,2]
    ...
    def evaluate(self,
        ...
        class_table_data_seq = [['Class'] + [m[1:] for m in metric] + ['Acc']]
        if self.CLASSES is None:
        ...
        ret_metrics_seq_round = [
            np.round(ret_metric * 100, 2) for ret_metric in ret_metrics_seq
        ]
        for i in range(num_classes):
            class_table_data.append([class_names[i]] +
                                    [m[i] for m in ret_metrics_round[2:]] +
                                    [ret_metrics_round[1][i]])
            class_table_data_seq.append([class_names[i]] +
                                        [m[:,i] for m in ret_metrics_seq_round[2:]] +
                                        [ret_metrics_seq_round[1][:,i]])
