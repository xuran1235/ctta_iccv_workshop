...
#     #drop_miou
#     drop_json_dict = []
#     drop_seq_path = os.path.join(res_path,'drop') 
#     # Loop over all files in the path
#     for file_named in os.listdir(drop_seq_path):
#         # Check if the file is a JSON file
#         if file_named.endswith(".json"):
#             # Extract the sequence ID from the file name
#             drop_seq_id = file_named.split(".")[0]
#             # Read the JSON file
#             with open(os.path.join(drop_seq_path, file_named), "r") as fd:
#                 contentd = json.load(fd)
#             # Append the content to the dictionary
#             drop_json_dict.append(float(contentd)) # json dict
#     average_drop = mean(drop_json_dict)
...
#         count_dict = OrderedDict()
#         for seq_id in seq_list:
#             if seq_id not in json_dict.keys():
#                 continue
#             eval_res = json_dict[seq_id]
#             for i, item in enumerate(eval_res):
#                 if i ==0:
...
#         table = AsciiTable(result_list)
#         # Open a file for writing
#         print("############Drop_Miou:",average_drop)
#         with open(os.path.join(res_path,save_name), 'a') as f:
#             # Redirect the output to the file
#             # print(table.table,'\n', file=f)
#             f.write("############Drop_Miou:"+str(average_drop)+'\n')
#             f.write("Evaluation on "+type+'\n')
#             f.write(table.table+'\n')
